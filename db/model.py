...
class Radrennen(db.Model):
    ...
    RadrennenID = db.Column(db.Integer, primary_key=True, unique=True)
...
class SportlerRadrennen(db.Model):
    ...
    RadrennenID = db.Column(db.ForeignKey('radrennen.RadrennenID'), index=True)
    Best_Zeit = db.Column(db.DateTime)

    radrennen = db.relationship('Radrennen', primaryjoin='SportlerRadrennen.RadrennenID == Radrennen.RadrennenID', backref='sportler_radrennens')
